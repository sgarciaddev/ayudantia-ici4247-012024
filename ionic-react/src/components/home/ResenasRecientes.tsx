...
import ResenaIndividual from '../resenas/ResenaIndividual';

const resenasRecientes: Array<{
  username: string;
  userImg: string;
  rating: number;
...
const ResenasRecientes: React.FC = () => {
  return (
    <IonList>
      ...
      {resenasRecientes.map((resena, i) => (
        <IonItem key={`home-resena-${i}`} className='mt-4'>
          <ResenaIndividual
            rating={resena.rating}
            review={resena.review}
            userImg={resena.userImg}
